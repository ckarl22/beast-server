...
// ─── COINBASE ORDER EXECUTION ─────────────────────────────────
function generateCoinbaseJWT(apiKey, apiSecret, method, path){
  try {
    ...
    const signature = sign.sign(apiSecret, 'base64url');
  ...
  return new Promise((resolve) => {
    try {
      ...
      const orderBody = JSON.stringify({
        ...
        order_configuration: {
          market_market_ioc: {
            [side === 'BUY' ? 'quote_size' : 'base_size']: side === 'BUY' ? dollarAmount.toFixed(2) : baseSize,
          }
        }
      });
      const jwt = generateCoinbaseJWT(apiKey, apiSecret, 'POST', '/api/v3/brokerage/orders');
      if(!jwt){ resolve({success:false, error:'JWT generation failed'}); return; }
      const req = https.request({
        ...
        res.on('end', () => {
          try {
            ...
            console.log('Coinbase response:', JSON.stringify(parsed).slice(0,200));
            if(parsed.success){
              resolve({success:true, orderId:parsed.success_response?.order_id, ticker, side, dollarAmount});
            } else {
              resolve({success:false, error:parsed.error_response?.message||parsed.error||'Order failed'});
            }
